...
`amazon-review-insight` 是一个 Codex agent skill，用于分析 Amazon US 单个 ASIN 的 Review，并生成中文 VOC 决策 HTML 报告。

它通过 Sorftime MCP 获取 `product_detail` 和 `product_reviews`，由当前 Codex/code agent 完成 Review 开放编码、VOC 主题归因和业务动作生成，脚本负责确定性解析、统计、契约检查、报告渲染、缓存和密钥扫描。
...
v0.1.0 sample report 使用真实 Sorftime 数据生成，默认测试 ASIN 为 `B0DHPN1DMJ`。样例报告不得包含 Sorftime key、环境变量值或运行时 token。
