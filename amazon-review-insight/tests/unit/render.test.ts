...
describe("report renderer", () => {
  it("renders required sections and highlighted theme details", async () => {
    ...
    expect(html).toContain('class="report-title">B0DHPN1DMJ Review VOC 决策报告</h1>');
    expect(html).toContain("id=\"scope\"");
    expect(html).toContain('<h2 class="section-title">数据范围与口径</h2>');
    expect(html).toContain("id=\"asin-metadata\"");
    expect(html).toContain("id=\"health\"");
    expect(html).toContain("id=\"voc-theme-map\"");
    expect(html).toContain('href="#scope">1. 数据范围与口径</a>');
    expect(html).toContain('href="#asin-metadata">2. ASIN 元数据</a>');
    expect(html).toContain('href="#health">3. Review 健康度</a>');
    expect(html).toContain('class="toc-group" open');
    expect(html).toContain(".toc-group:not([open]) > .toc-sublist");
    expect(html).toContain('href="#key-insights">4. 关键结论</a>');
    expect(html).toContain("toc-sublist toc-key-insights");
    expect(html).toContain('href="#key-insight-audience">4.1 人群</a>');
    expect(html).toContain('href="#key-insight-dissatisfaction">4.8 不满意点</a>');
    expect(html).toContain('href="#voc-theme-map">5. VOC 主题地图</a>');
    expect(html).toContain("toc-sublist toc-voc-themes");
    expect(html).toContain('href="#voc-theme-group-positive">5.1 正向主题</a>');
    expect(html).toContain('href="#voc-theme-group-negative">5.2 负向主题</a>');
    expect(html).toContain('href="#voc-theme-group-unmet">5.3 未满足的机会点</a>');
    expect(html).toContain('href="#actions">6. 机会矩阵与业务动作</a>');
    expect(html).toContain('href="#limits">7. 限制与 Checkpoint</a>');
    const scopeStart = html.indexOf('id="scope"');
    const scopeEnd = html.indexOf('</section>', scopeStart);
    const scopeHtml = html.slice(scopeStart, scopeEnd);
    expect(scopeHtml).not.toContain("Review VOC 决策报告");
    expect(html).toContain("product-metadata-card");
    expect(html).toContain("product-image");
    expect(html).toContain("Ikarao Karaoke Machine");
    expect(html).toContain("ASIN 总评论数");
    expect(html).toContain("所属大类");
    expect(html).toContain("所属细分类目");
    expect(html).toContain("上架时间");
    expect(html).toContain("样本评论最早日期");
    expect(html).toContain("样本评论最新日期");
    expect(html).toContain("样本各星级分布");
    expect(html).not.toContain("<h3>星级分布</h3>");
    expect(html).not.toContain("正文存在率");
    expect(html).not.toContain("日期存在率");
    ...
    expect(html).toContain("--min-width: 1200px");
    expect(html).toContain("--detail-max-width: 1120px");
    expect(html).toContain("width: min(100%, var(--max-width))");
