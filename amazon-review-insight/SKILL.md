...
`原始评论` sheet 前五列必须为 `ASIN`、`评论日期`、`星级`、`title`、`text`。`Review编码层` sheet 前五列也必须为 `ASIN`、`评论日期`、`星级`、`title`、`text`，后续列再展开编码单元 ID、编码维度、人群、场景、用户任务、购买理由、用户期望、实际体验、满意点、不满意点、证据原文、开放标签和置信度。
