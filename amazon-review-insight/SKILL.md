...
9. 主报告页的关键结论和 VOC 主题卡片展示 3 条不同的完整代表性 Review 原文和完整中文翻译；主报告和详情页都必须使用句子级证据高亮，不再生成或保留关键词高亮字段。
...
- 修改版本号、发布流程或已发布版本行为前，读取 `references/specs/version-lifecycle.md`。

## 工作流

1. 校验用户只提供一个合法 ASIN；站点固定为 Amazon US。
2. 使用 Sorftime MCP 调用 `product_detail` 和 `product_reviews`，参数 `amzSite=US`，`reviewType=Both`。
3. 将 Sorftime 中文字段规范化为标准 Review 对象，并保留原始字段到 `raw`；将 `product_detail` 规范化为 `product_metadata`，用于报告 ASIN 元数据模块。
...
1. 已发布版本必须冻结；任何已经打 tag 并推送到 GitHub 的版本，都不得再被修改、移动、强推或覆盖。
2. 后续任何行为、样式、契约、测试、文档或输出口径修改，都必须通过新版本承载。
3. 当前最新已发布版本是 `v0.3.2`，状态为 `released`，已冻结；后续修改必须进入高于 `v0.3.2` 的新版本。
4. 修复已发布版本的问题时，不回写原 tag；创建下一个 patch 版本，并在 CHANGELOG 中说明。
5. 发布前必须同步 `package.json`、`package-lock.json`、`SKILL.md`、相关 contract/spec、checkpoint、BDD、eval、tests 和已安装 skill。

## 报告结构

交付文件必须使用 `YYYYMMDD-ASIN-suffix` 命名，例如 `20260616-B0CR1R7FKP-review-insight-report.html` 和 `20260616-B0CR1R7FKP-review-coding.xlsx`。

HTML 报告必须包含：

报告总标题必须独立显示为 `{ASIN} Review VOC 决策报告`，不得替代任何一级章节标题。

1. 数据范围与口径。
2. ASIN 元数据。
3. Review 健康度。
4. 关键结论，且必须覆盖人群、场景、用户任务、购买理由、用户期望、实际体验、满意点、不满意点；左侧一级导航和二级导航必须带序号，左侧导航必须在“4. 关键结论”下展示八类洞察二级导航，默认展开并支持折叠；每个结论必须是一行一个默认展开的可折叠块，并包含类型分布表；正文标题右上角必须有问号 tooltip 解释关键结论与 VOC 主题地图的区别；关键结论卡片整体可点击并在新标签页打开该关键结论详情页。
5. VOC 主题地图；左侧一级导航和二级导航必须带序号，左侧导航必须在“5. VOC 主题地图”下展示正向主题、负向主题、未满足的机会点三类二级导航，默认展开并支持折叠；正文必须按这三类分组穷举全部前台 VOC 主题，每个主题只能归入一个主分组；每个主题必须是一行一个默认展开的可折叠块，并展示主题内观点分布；正文标题右上角必须有问号 tooltip 解释 VOC 主题地图与关键结论的区别。
6. 关键结论详情页和 VOC 主题详情页。
7. 详情页内筛选：点击关键结论或 VOC 主题卡片后，当前主报告页不得滚动跳转；浏览器必须在新标签页打开同一 HTML 内的对应详情视图。新标签页必须进入独立路由模式，只展示当前详情，不展示目录、主报告其它章节或其它详情。关键结论详情页顶部 sticky 面板必须展示所属关键结论和类型筛选入口；VOC 主题详情页顶部 sticky 面板必须把所属 VOC 主题卡片和观点筛选入口合并为同一个卡片，避免上下两个主题块割裂或重复；主题区必须展示核心问题、归因假设、业务含义和运营动作；下方默认展示相关全量 Review 原文、完整中文翻译和句子级证据高亮。用户在该新标签页内点击某个类型或观点后，当前标签页筛选出对应的全量 Review。
8. 机会矩阵与业务动作清单，必须按业务方向拆成多个默认展开的可折叠方向块，不得把所有动作扔进一个横向大表。
9. 限制说明与 checkpoint 状态。

“数据范围与口径”模块的正文章节标题必须显示为“数据范围与口径”，只保留站点、ASIN、数据来源、抓取时间、已知缺失字段；不得把 `{ASIN} Review VOC 决策报告` 放在该模块标题位置。“ASIN 元数据”必须展示 ASIN、品牌、标题、主图、价格、星级、ASIN 总评论数、分类、所属大类、所属细分类目、上架时间。“Review 健康度”只保留样本平均星级、4-5 星占比、1-3 星占比、样本评论最早日期、样本评论最新日期、样本各星级分布；不得展示正文存在率或日期存在率。

主报告一级章节标题下方不得渲染说明文字；必要解释通过标题问号 tooltip 或具体卡片字段承载。摘要 chip 必须带中文标签，例如“提及：”“置信度：”“主题类型：”“严重度：”“运营优先级：”，不得只展示裸值。主报告内每个关键结论和 VOC 主题卡片必须展示 3 条不同的完整代表性 Review 原文和完整中文译文；当相关评论不足 3 条时展示全部可追溯评论；不得把关键词、短句或 evidence 片段当作代表性原文。HTML 报告只按电脑端桌面布局设计，主报告最大宽度为 1440px，并在 1200px 到 1440px 的电脑端窗口内自然收缩；不再维护移动端媒体查询或移动端特化样式。
...
关键结论卡片必须整体可点击，点击卡片任意非链接位置时，当前主报告页不得滚动跳转；浏览器必须在新标签页打开同一个 HTML 内的该关键结论详情视图。关键结论详情视图必须隐藏主报告其它章节、目录和其它详情，顶部 sticky 面板展示关键结论摘要和类型筛选入口。默认展示支撑该关键结论的全部相关 Review 原文、完整中文翻译和句子级证据高亮；点击类型后，在当前详情页筛选对应类型相关的全量 Review。

VOC 主题卡片必须整体可点击，点击卡片任意非链接位置时，当前主报告页不得滚动跳转；浏览器必须在新标签页打开同一个 HTML 内的该主题详情视图。主题详情视图必须隐藏主报告其它章节、目录和其它主题详情，避免向上或向下滚动看到无关内容。主题详情页展示该主题相关完整 Review 原文、完整中文翻译，并对原文和译文中支撑主题的完整证据句使用语义颜色高亮。

详情 Review 必须使用 `evidence_sentences[]`，每条证据句包含 `original`、`translation`、`evidence_type` 和可选 `target`。`evidence_type` 取值为 `positive`、`negative`、`opportunity`、`context`，分别用于正向证据、负向证据、机会证据和背景证据。不得继续生成、保留、导出或渲染 `highlight_terms`、`translation_highlight_terms` 这类关键词高亮字段。

证据句高亮必须按当前上下文筛选：关键结论卡片只高亮支撑该关键结论或类型分布的句子，VOC 主题卡片只高亮支撑该主题的句子，观点筛选状态只高亮支撑该观点的句子。同一条 Review 出现在不同结论或主题下时，必须根据当前卡片重新选择证据句，不得把音质、价格、故障等其它目标的证据句带入“场景”“人群”等无关结论。
...
2. `skill_version` 与 release 版本一致，且已发布版本不得再被修改。
