...
- 新增交付文件命名规范：`YYYYMMDD-ASIN-review-insight-report.html` 和 `YYYYMMDD-ASIN-review-coding.xlsx`。
- 主报告新增 `ASIN 元数据`章节，展示 ASIN、品牌、标题、主图、价格、星级、ASIN 总评论数、分类、所属大类、所属细分类目和上架时间。
- `数据范围与口径` 收敛为站点、ASIN、数据来源、抓取时间和已知缺失字段。
- 报告总标题与 `数据范围与口径` 章节标题分离，避免把 `{ASIN} Review VOC 决策报告` 当作第 1 章标题。
- `Review 健康度` 收敛为样本平均星级、4-5 星占比、1-3 星占比、样本评论最早/最新日期和样本各星级分布，移除正文存在率和日期存在率展示。
- 主报告桌面布局提升到 1440px 最大宽度，并在 1200px 到 1440px 电脑端窗口内自然收缩；详情页提升到 1120px 阅读宽度。
- 新增版本生命周期规范：已发布版本必须冻结，后续任何行为、样式、契约、测试、文档或输出口径修改都通过新版本承载。
- 新增版本生命周期 BDD、checkpoint 和测试，防止已发布 tag 被移动、覆盖或在旧版本上继续改动。

## v0.3.1

- 关键结论卡片支持像 VOC 主题卡片一样在新标签页打开详情页，并在详情页内按类型筛选全量相关 Review。
- VOC 主题地图改为 `正向主题 / 负向主题 / 未满足的机会点` 三分组，三组合计穷举全部前台主题。
- 主报告关键结论和 VOC 主题卡片中的证据区改为展示 3 条不同的完整代表性 Review 原文和完整中文译文。
- 高亮逻辑改为句子级证据高亮，按正向、负向、机会、背景使用语义颜色；移除 `highlight_terms` 和 `translation_highlight_terms` 关键词字段。
- 契约检查、BDD、checkpoint、eval 和 renderer 测试同步覆盖 v0.3.1 行为。
...
- 观点详情页顶部 sticky 展示所属 VOC 主题卡片，下方展示该观点相关全量 Review 原文、完整中文翻译和证据高亮。
